...
proof-
  ...
  also have "... = (q * pderiv p - p * pderiv q) div q"
      by (rule div_diff[symmetric], simp_all)
...
proof-
  from bezout obtain r s where rs: "d = r * p + s * pderiv p" 
      unfolding d_def by (subst (asm) eq_commute, blast)
  def t \<equiv> "p div d" 
...
hide_fact poly_div_gcd_squarefree_aux
