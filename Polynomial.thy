...
subsection {* Alternative definition of evaluation function *}

lemma poly_altdef: 
  fixes x :: "'a::{comm_semiring_0, comm_monoid_mult, power}"
  shows "poly p x = (\<Sum>i\<le>degree p. coeff p i * x ^ i)"
  ...
  case (pCons a p)
    ...
    proof (cases "p = 0")
      ...
      also have "a + x * (\<Sum>i\<le>degree p. coeff p i * x ^ i) = 
          ...
          by (simp add: algebra_simps setsum_right_distrib coeff_pCons)
...
lemma poly_div_const[simp]: "a \<noteq> 0 \<Longrightarrow> p div [:a:] = smult (inverse a) p"
proof-
  assume "a \<noteq> 0"
  hence "[:a:] \<noteq> 0" by simp
  from `a \<noteq> 0` have "p = smult (inverse a) p * [:a:]" by simp
  hence "p div [:a:] = smult (inverse a) p * [:a:] div [:a:]" by simp
  also from `[:a:] \<noteq> 0` have "... = smult (inverse a) p"
      by (rule div_mult_self2_is_id)
  finally show ?thesis .
qed
...
begin
  ...
  definition [simp]: "normalisation_factor_poly p \<equiv> [:coeff p (degree p):]"

  instance proof
    case (goal1 b a)
      thus ?case using degree_mod_less by auto
  next
    case (goal2 b a)
      thus ?case by (auto intro: dvd_imp_degree_le)
  next
    case (goal3 a) 
      hence "coeff a (degree a) \<noteq> 0" using leading_coeff_0_iff by simp
      hence "1 = normalisation_factor a * [:inverse (coeff a (degree a)):]"
          by (simp add: one_poly_def)
      thus ?case unfolding is_unit_def by (rule dvdI)
  next
    case (goal4 a b)
      thus ?case using coeff_mult_degree_sum[of a b] degree_mult_eq[of a b]
          by (cases "a \<noteq> 0 \<and> b \<noteq> 0") auto
  next
    case (goal5 a)
      hence [simp]: "degree a = 0" using dvd_imp_degree_le[of a 1]
          unfolding is_unit_def by (cases "a = 0", simp, simp)
      then obtain k where "a = [:k:]" by (cases a, simp split: split_if_asm)
      thus "normalisation_factor a = a" by simp
  next
    case goal6
      show ?case by simp
  qed
...
begin
  function gcd_poly :: "'a::field poly \<Rightarrow> 'a poly \<Rightarrow> 'a poly" where
  "gcd_poly x y = (if y = 0 then 
                     if x = 0 then 0 else smult (inverse (coeff x (degree x))) x 
                   else
                     gcd_poly y (x mod (y :: _ poly)))"
      by (pat_completeness, simp)
  termination 
    apply (relation "measure (euclidean_size \<circ> snd)", rule wf_measure)
    apply (clarsimp simp del: euclidean_size_poly_def)
    done

  declare gcd_poly.simps [simp del]

  definition lcm_poly :: "'a::field poly \<Rightarrow> 'a poly \<Rightarrow> 'a poly" where
  "lcm_poly a b = 
     a * b div smult (coeff a (degree a) * coeff b (degree b)) (gcd a b)"

  definition "Lcm_poly \<equiv> Lcm_eucl :: 'a::field poly set \<Rightarrow> 'a poly"
  definition "Gcd_poly \<equiv> Gcd_eucl :: 'a::field poly set \<Rightarrow> 'a poly"

  instance proof
    show [simp]: "(gcd :: 'a::field poly \<Rightarrow> _) = gcd_eucl"
    proof (intro ext)
      fix a :: "'a poly" and b :: "'a poly"
      show "gcd a b = gcd_eucl a b"
        by (induction a b rule: gcd_eucl.induct)
           (subst gcd_poly.simps, subst gcd_eucl.simps, simp_all)
    qed

    show "(lcm :: 'a::field poly \<Rightarrow> _) = lcm_eucl"
    proof (intro ext)
      fix a :: "'a poly" and b :: "'a poly"
      show "lcm a b = lcm_eucl a b" unfolding lcm_poly_def lcm_eucl_def
        by (cases "a = 0 \<or> b = 0") (auto simp: coeff_mult_degree_sum degree_mult_eq)
    qed
  qed (simp_all add: Lcm_poly_def Gcd_poly_def)
end

lemma poly_gcd_monic: 
  "coeff (gcd x y) (degree (gcd x y)) = (if x = 0 \<and> y = 0 then 0 else 1)"
  using normalisation_factor_gcd[of x y]
      by (auto split: split_if_asm simp: one_poly_def)

lemma poly_lcm_monic: 
  "coeff (lcm x y) (degree (lcm x y)) = (if x = 0 \<or> y = 0 then 0 else 1)"
  using normalisation_factor_lcm[of x y]
      by (auto split: split_if_asm simp: one_poly_def)
...
proof(clarify)
  ...
  hence "[:-x,1:] dvd gcd p q" by (rule gcd_greatest)
